...
typedef enum {
    CPXRigidBodyTypeStatic,
    CPXRigidBodyTypeKinematic,
    CPXRigidBodyTypeDynamic
} CPXRigidBodyType;

typedef struct PXMatrix4 (^CPXMotionStateTransformGetBlock)(void);
typedef void (^CPXMotionStateTransformSetBlock)(struct PXMatrix4);

@class CPXCollisionShape;

@interface CPXRigidBody : NSObject

@property (nonatomic) _Nullable CPXMotionStateTransformGetBlock onGetVisualTransform;
@property (nonatomic) _Nullable CPXMotionStateTransformSetBlock onSetVisualTransform;
...
@property (nonatomic, assign) struct PXVector3 eulerOrientation;
...
@property (nonatomic, assign) struct PXVector3 linearVelocity;

/// The current angular velocity of the rigid body in the simulation
@property (nonatomic, assign) struct PXVector3 angularVelocity;
...
@property (nonatomic, assign) struct PXVector3 linearVelocityFactor;
...
@property (nonatomic, assign) struct PXVector3 angularVelocityFactor;
...
@property (nonatomic, assign) struct PXVector3 centerOfMass;
...
@property (nonatomic, readonly) CPXRigidBodyType rigidBodyType;

- (instancetype)initWithCollisionShape: (CPXCollisionShape *)collisionShape rigidBodyType: (CPXRigidBodyType)rigidBodyType mass:(float)mass;

/// Removes all forces currently applied to this rigid body
- (void)clearForces;

// Applies a force or impulse to the body at its center of mass
- (void)applyForce: (struct PXVector3)force impulse: (BOOL)impulse;

// Applies a net torque or a change in angular momentum to the body.
- (void)applyTorque: (struct PXVector3)torque impulse: (BOOL)impulse;
